...
namespace TrackDaNutzz.Services.Import
{
    public class ImportService : IImportService
    {
        ...
        public bool Add(HandDto handDto)
        {
            ...
            int tableId = this.AddTable(handDto.TableDto);
            long handId = this.AddHand(handDto, boardId, tableId);
            Dictionary<string, int> playerIdsByName = this.AddPlayers(handDto, handId);
            List<long> bettingActionsIds = this.AddBettingActions(handDto, handId, playerIdsByName);
            this.context.SaveChanges();
            //TODO: Add Mapping Table HandPlayerBettingAction (for mapping bettingAction order)!!!!!!!!!!
            return true;
        }

        private int AddTable(TableDto tableDto)
        {
            int tableSize = 0;
            if (tableDto.TableSize.EndsWith("-max"))
            {
                tableSize = int.Parse(tableDto.TableSize.Replace("-max", ""));
            }
            else if (tableDto.TableSize == "Heads-Up")
            {
                tableSize = 2;
            }
            Table table = this.context.Tables.SingleOrDefault(t => t.Name == tableDto.TableName && t.Size == tableSize);
            if (table != null)
            {
                return table.Id;
            }
        ...
        private Dictionary<string, int> AddPlayers(HandDto handDto, long handId)
        {
            ...
            {
                Player player = this.context.Players.SingleOrDefault(p => p.Name == seatInfoDto.PlayerName);
                if (player == null)
                {
                    player = new Player()
                    {
                ...
                if (handPlayer == null)
                {
                    handPlayer = new HandPlayer()
                    {
                        ...
                        FinalStack = this.CalculateFinalStack(handDto, seatInfoDto),
        ...
        private decimal CalculateFinalStack(HandDto handDto, SeatInfoDto seatInfoDto)
        {
            decimal betMoney = handDto.BettingActionsByRoundListDto.BettingActionsByRoundDtos
                        ...
                        .ToList()
                        .Sum();
            decimal collectedMoney = handDto.CollectMoneyListDto.CollectMoneyDtos.Where(x => x.PlayerName == seatInfoDto.PlayerName).Sum(x=>x.Value);
        ...
        {
            Stake stake = this.context.Stakes.SingleOrDefault(s => s.BigBlind == handInfoDto.BigBlind && s.SmallBlind == handInfoDto.SmallBlind);
            if (stake != null)
            {
                return stake.Id;
            }
            ...
            foreach (var bettingActionsByRoundDto in handDto.BettingActionsByRoundListDto.BettingActionsByRoundDtos)
            {
                foreach (var bettingActionDto in bettingActionsByRoundDto.BettingActionDtos)
                {
                    BettingAction bettingAction = new BettingAction()
                    {
                        ...
                        Type = bettingActionDto.Action.Contains("bets") || bettingActionDto.Action.Contains("raises")
                    };
